...
function redirectToStripe() {
  ...
  // Save QR code data in localStorage
  localStorage.setItem("qrText", qrText);
  console.log("Redirecting to Stripe with QR Text:", qrText);

  // Redirect to Stripe payment link with a custom success URL
  const stripePaymentLink = "https://buy.stripe.com/test_9AQ5o60a43oz0XCbII";
  const successUrl = `${window.location.origin}?paid=true`; // Ensure your domain is used
  window.location.href = `${stripePaymentLink}?success_url=${encodeURIComponent(successUrl)}`;
}
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (isPaid) {
    const qrText = localStorage.getItem("qrText"); // Retrieve QR code data
    if (qrText) {
      // Display the thank-you message and QR code
      ...
      `;
      generateQRCodeWithoutWatermark(qrText); // Generate QR code without watermark
    } else {
